...
class Dataset(torch.utils.data.Dataset):
	def __init__(self, 	data_dir=None, 
		...
		# load data
		self._sample_id_list, self._samples_arr, self._labels_arr = self.prepare_data()
	...
	def prepare_data(self):
		...
		for i in range(num_data_files):
			...
			num_data_points = num_vectors - N + 1
			...
			for j in range(num_data_points):
				...
				sample_id_list.append((file_id,j))

		# return samples and labels
		return sample_id_list, np.array(sample_list, dtype=np.float32), np.array(label_list, dtype=np.uint8)
